...
_Last updated: 2026-09-24, #34 (the computer now thinks ahead in the background and plays strongly)._
...
- **Against the computer**: choose "You start" or "Computer starts"; whoever starts plays red. Click a column on your turn, and the computer answers after about a second. It now thinks ahead: it takes its wins, blocks yours and is hard to beat. While it thinks, the page keeps reacting: "New game" or changing the mode or who starts cancels its move at once. Choosing an easier level comes next (#35). The line above the board says "Your turn", "Computer is thinking…", "You win!", "Computer wins!" or "Draw!".
...
- Playing against a computer that looks ahead for about a second per move (in the background, so the page never freezes), with you or the computer starting. If the background search ever fails, the computer still moves, using a quicker search, and the error appears in the browser console.
...
- Behind the scenes, for the stronger computer levels to come: a fast internal board that can try out about 16 million positions per second, a score for how good a position looks (counting the lines of four each player can still complete, plus discs in the centre column), and a search that looks ahead as many moves as its time allows (from the start of a game, 8 moves take about 6 milliseconds, 12 moves about 0.14 seconds), finds forced wins and says what it could prove (for example "Red wins in 2 moves" or a draw). The computer opponent uses it since #34; the analysis (#22) will show what it proves.

## Next up

Milestone 6 (bot levels):

1. #35 Choose the computer's level: Beginner, Easy, Medium, Hard, Expert
2. #36 Measure that each level beats the one below
3. #22 Show the best move and the expected result on request
