...
**Play online** is not playable yet: it connects two PCs, but moves are not sent until the next issue.
...
- Groundwork for online play: two browsers can open a direct connection to each other (via PeerJS, no server of our own), and there is a fixed, checked message format for what they send each other (moves, rematch, keep-alive), so broken or outdated data is ignored.
- "Play online" shows a link with a "Copy link" button. Opening it on another PC (or tab) connects the two; both then show "Connected", or ask to reload if the two pages are different versions. The board stays locked for now.

## Next up

1. #20 Play moves live between two PCs
2. #21 Handle lost connections, bad links and rematches online
